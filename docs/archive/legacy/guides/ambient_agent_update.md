## ✅ Plano de Atualização para o Agente AMBIENT

Este guia contém todos os códigos e queries corrigidos e prontos para serem copiados e colados no workflow do Agente AMBIENT.
...
-- ATENÇÃO: Altere para 'agent_ambient_memory'
UPDATE agent_ambient_memory SET is_latest = false WHERE territory_id = '{{ $json.territory_id }}';

INSERT INTO agent_ambient_memory (id, territory_id, analysis_date, analysis_content, analysis_summary, confidence_score, time_range, indicators_used, data_sources, is_latest, metadata)
...
```sql
-- V2 - CORRIGIDO
-- ATENÇÃO: Altere para 'agent_ambient_learning_evolution' e 'ambient'
INSERT INTO agent_ambient_learning_evolution (agent_id, learning_type, source_analysis_ids, learning_content, confidence_score, metadata)
SELECT
  'ambient' AS agent_id,
...
let summary = 'Análise ambiental completa gerada.';
try {
...
const response = {
  ...
  metadata: {
    ...
    analysis_type: 'environmental_comprehensive',
